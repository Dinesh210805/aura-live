...
_EndSensitivity = None    # Optional: end-of-speech sensitivity tuning
...
try:
    ...
    try:
        ...
        try:
            from google.genai.types import EndSensitivity as _EndSensitivity  # type: ignore[assignment]
        except ImportError:
            pass  # _EndSensitivity stays None (declared at module level above)
...
async def handle_live_websocket(
    ...
    # VAD settings:
    #   prefix_padding_ms=160  — faster speech-start commitment for low latency.
    #   silence_duration_ms=650 — quicker turn-end than 1200 ms while tolerating
    #                             natural speech pauses.
    #   start_of_speech_sensitivity=HIGH and end_of_speech_sensitivity=HIGH
    #                             to reduce delayed start/end detection.
    ...
    if _VAD_TYPES_AVAILABLE:
        try:
            ...
            if _StartSensitivity is not None:
                _aad_kwargs["start_of_speech_sensitivity"] = _StartSensitivity.START_SENSITIVITY_HIGH
            if _EndSensitivity is not None:
                _aad_kwargs["end_of_speech_sensitivity"] = _EndSensitivity.END_SENSITIVITY_HIGH
            ...
            logger.info("[/ws/live] VAD: RealtimeInputConfig applied (160 ms prefix, 650 ms silence, HIGH start/end sensitivity, barge-in ON)")
        except Exception as _vad_exc:
            logger.warning(f"[/ws/live] Could not build RealtimeInputConfig: {_vad_exc} — using defaults")

    # Generated by Copilot: per-turn latency instrumentation for /ws/live.
    turn_metrics = {
        "turn_index": 0,
        "turn_started_ts": None,
        "audio_first_ts": None,
        "audio_last_ts": None,
        "client_end_turn_ts": None,
        "first_input_transcript_ts": None,
        "first_output_transcript_ts": None,
        "first_model_audio_ts": None,
        "turn_complete_ts": None,
        "audio_chunks_in": 0,
    }

    def _start_turn_if_needed(trigger: str) -> None:
        if turn_metrics["turn_started_ts"] is None:
            turn_metrics["turn_started_ts"] = time.perf_counter()
            turn_metrics["turn_index"] += 1
            logger.info(f"[/ws/live] turn_start #{turn_metrics['turn_index']} via {trigger}")

    def _reset_turn_metrics_for_next_turn() -> None:
        turn_metrics["turn_started_ts"] = None
        turn_metrics["audio_first_ts"] = None
        turn_metrics["audio_last_ts"] = None
        turn_metrics["client_end_turn_ts"] = None
        turn_metrics["first_input_transcript_ts"] = None
        turn_metrics["first_output_transcript_ts"] = None
        turn_metrics["first_model_audio_ts"] = None
        turn_metrics["turn_complete_ts"] = None
        turn_metrics["audio_chunks_in"] = 0
        ...
        try:
            while True:
                ...
                if msg_type == "audio_chunk":
                    ...
                    _start_turn_if_needed("audio_chunk")
                    now = time.perf_counter()
                    if turn_metrics["audio_first_ts"] is None:
                        turn_metrics["audio_first_ts"] = now
                    turn_metrics["audio_last_ts"] = now
                    turn_metrics["audio_chunks_in"] += 1
                    ...
                    if text:
                        _start_turn_if_needed("text_command")
                    ...
                    # Client explicitly marks end-of-utterance.
                    # In addition to server VAD, forward an activity-end signal when
                    # available to reduce waiting for delayed VAD closure.
                    if turn_metrics["turn_started_ts"] is None:
                        logger.debug("[/ws/live] end_turn ignored (no active turn)")
                        continue
                    turn_metrics["client_end_turn_ts"] = time.perf_counter()
                    signaled = False
                    try:
                        if hasattr(live_queue, "send_audio_stream_end"):
                            live_queue.send_audio_stream_end()  # type: ignore[attr-defined]
                            signaled = True
                            logger.info("[/ws/live] end_turn mapped to send_audio_stream_end()")
                    except Exception as _ase:
                        logger.debug(f"[/ws/live] send_audio_stream_end unavailable: {_ase}")

                    if not signaled:
                        try:
                            if hasattr(live_queue, "send_activity_end"):
                                live_queue.send_activity_end()  # type: ignore[attr-defined]
                                signaled = True
                                logger.info("[/ws/live] end_turn mapped to send_activity_end()")
                        except Exception as _ae:
                            logger.debug(f"[/ws/live] send_activity_end unavailable: {_ae}")

                    if not signaled:
                        logger.info("[/ws/live] end_turn received (no explicit queue end-signal method available)")
    ...
    async def send_to_device() -> None:
        ...
        async def _flush_response(source: str) -> None:
            ...
            flushed_turn_index = turn_metrics["turn_index"]
            ...
            def _delta_ms(start_key: str, end_key: str | None = None) -> int | None:
                start = turn_metrics.get(start_key)
                end = now if end_key is None else turn_metrics.get(end_key)
            ...
            logger.info(
                "[/ws/live] timing "
                f"turn=#{turn_metrics['turn_index']} source={source} "
                f"chunks_in={turn_metrics['audio_chunks_in']} "
                f"audio_to_input_tx_ms={_delta_ms('audio_first_ts', 'first_input_transcript_ts')} "
                f"audio_to_ai_audio_ms={_delta_ms('audio_first_ts', 'first_model_audio_ts')} "
                f"end_turn_to_input_tx_ms={_delta_ms('client_end_turn_ts', 'first_input_transcript_ts')} "
                f"end_turn_to_ai_audio_ms={_delta_ms('client_end_turn_ts', 'first_model_audio_ts')} "
                f"end_turn_to_turn_complete_ms={_delta_ms('client_end_turn_ts', 'turn_complete_ts')} "
                f"audio_span_ms={_delta_ms('audio_first_ts', 'audio_last_ts')} "
                f"flush_after_turn_complete_ms={_delta_ms('turn_complete_ts')}"
            )
            ...
            ai_audio_sent = False
            # A new turn can start while this coroutine awaits websocket I/O.
            # Only reset metrics if we are still on the same flushed turn.
            if turn_metrics["turn_index"] == flushed_turn_index:
                _reset_turn_metrics_for_next_turn()
        ...
        try:
            async for event in runner.run_live(
                ...
                if input_text:
                    if turn_metrics["first_input_transcript_ts"] is None:
                        turn_metrics["first_input_transcript_ts"] = time.perf_counter()
                ...
                if transcript_text:
                    if turn_metrics["first_output_transcript_ts"] is None:
                        turn_metrics["first_output_transcript_ts"] = time.perf_counter()
                ...
                if audio_chunks:
                    if turn_metrics["first_model_audio_ts"] is None:
                        turn_metrics["first_model_audio_ts"] = time.perf_counter()
                    ai_audio_sent = True
                ...
                if is_turn_complete:
                    if turn_metrics["turn_complete_ts"] is None:
                        turn_metrics["turn_complete_ts"] = time.perf_counter()
