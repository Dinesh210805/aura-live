...
def analyze_ui_node(state: TaskState) -> Dict[str, Any]:
    ...
    try:
        ...
        # Convert to UIElement objects - handle both real and mock data formats
        from services.real_accessibility import RealUIElement
        from utils.types import UIElement

        element_objects = []
        conversion_errors = []
        is_real_data = (
            isinstance(ui_elements_data, list)
            and len(ui_elements_data) > 0
            and isinstance(ui_elements_data[0], RealUIElement)
        )

        def parse_bounds(bounds_data) -> List[int]:
            """Parse bounds from various formats into [left, top, right, bottom]."""
            ...
            elif isinstance(bounds_data, (list, tuple)) and len(bounds_data) >= 4:
                return [int(bounds_data[i]) for i in range(4)]
            return [0, 0, 0, 0]  # Fallback

        if is_real_data:
            # Handle real UI elements from accessibility service
            for i, real_elem in enumerate(ui_elements_data):
                try:
                    # Convert RealUIElement to UIElement format
                    bounds = parse_bounds(real_elem.bounds)
                    ui_element = UIElement(
                        id=real_elem.id or f"element_{i}",
                        text=real_elem.text or "",
                        content_desc=real_elem.contentDescription or "",
                        class_name=real_elem.className or "unknown",
                        package_name=real_elem.packageName or "",
                        resource_id=real_elem.viewId or "",
                        bounds=bounds,
                        clickable=real_elem.isClickable,
                        scrollable=real_elem.isScrollable,
                        editable=real_elem.isEditable,
                        enabled=real_elem.isEnabled,
                        focusable=False,  # Default value
                        selected=False,  # Default value
                        checkable=False,  # Default value
                        checked=False,  # Default value
                    )
                    element_objects.append(ui_element)
                except Exception as elem_error:
                    logger.warning(
                        f"Failed to convert real UI element {i}: {elem_error}"
                    )
                    conversion_errors.append(f"Real element {i}: {str(elem_error)}")
                    continue
        else:
            # Handle mock UI elements (fallback)
            for i, elem in enumerate(ui_elements_data):
                try:
                    ui_element = UIElement.from_android_element(elem)
                    element_objects.append(ui_element)
                except Exception as elem_error:
                    logger.warning(
                        f"Failed to convert mock UI element {i}: {elem_error}"
                    )
                    conversion_errors.append(f"Mock element {i}: {str(elem_error)}")
                    continue
        ...
        output_details = {
            ...
            "raw_elements_count": len(ui_elements_data),
            "parsed_elements_count": len(element_objects),
            "conversion_errors": conversion_errors[:5],  # Limit to first 5 errors
            "data_type": data_type,
            "real_device_connected": is_real_data,
        }
        ...
        update_workflow_step(
            ...
            node_name,
            "completed",
            f"{data_type.title()} UI analysis completed: {len(element_objects)} elements processed",
        ...
        # Note: Legacy screen description generation removed - this is now handled
        # by the Perception Controller's Screen Reader Agent via perception_node.
        
        # Return empty result - perception data should come from controller
        return {
            "ui_screenshot": None,
            "ui_tree": None,
            "ui_elements": [],
            "status": "pending_perception",
        }
...
def plan_node(state: TaskState) -> Dict[str, Any]:
    ...
    try:
        ...
        # Check for permission dialog - agent should wait for user action
        is_permission_dialog = state.get("is_permission_dialog", False)
        permission_dialog_type = state.get("permission_dialog_type")
        
        if is_permission_dialog:
            logger.warning(
                f"ðŸš« Permission dialog detected (type={permission_dialog_type}). "
                "Returning wait_for_user action instead of planning."
            )
            ...
            update_workflow_step(
                ...
                node_name,
                "completed",
                f"Waiting for user to handle {permission_dialog_type} permission dialog",
                execution_time=execution_time,
            )
            
            # Return a special plan that tells executor to wait
            wait_plan = [{
                "action": "wait_for_user_permission",
                "type": "wait",
                "dialog_type": permission_dialog_type,
                "description": f"Waiting for user to grant or deny {permission_dialog_type} permission",
                "wait_seconds": 5,  # Wait 5 seconds then retry perception
        ...
        error_details = {
            ...
            "available_data": {
                ...
                "has_ui_screenshot": bool(state.get("ui_screenshot")),
                "ui_elements_count": len(state.get("ui_elements", [])),
            },
        }
